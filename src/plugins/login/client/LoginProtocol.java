...
public class LoginProtocol implements Protocol {
	...
	public void interpret(JSONObject msg, AbstractController c) throws ClinSysException {
		...
		BASE_COMMANDS cmdNotify;
		LOGIN_COMMANDS cmdAnswer;
		MESSAGE_TYPES type;
		...
		if (receiver.equals(lc.getPluginName())){
			...
			switch(type){
			case NOTIFY:
				cmdNotify =  BASE_COMMANDS.valueOf(msg.getString("Command"));
				switch(cmdNotify){
				case CONNECTED:
					lc.requestLogin(msg.getLong("TemporaryId"));
					break;
				default:
					break;
				}
				break;
			case ANSWER:
				cmdAnswer =  LOGIN_COMMANDS.valueOf(msg.getString("Command"));
				switch(cmdAnswer){
				case LOGIN:
					lc.login(msg);
					break;
			...
			tempController = lc.getClientController().getController(receiver);
