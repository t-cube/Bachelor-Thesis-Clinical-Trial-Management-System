 ...
 *
 */
public class RightsProtocol implements Protocol {
	...
	public void interpret(JSONObject msg, AbstractController c) throws ClinSysException {
		...
		if (c.getPluginName().equals("RightsController")){
			rc = (RightsController) c;
		}else{
			// error message
		}
		
		if (receiver.equals(rc.getPluginName())){
			type = (MESSAGE_TYPES) msg.get("MessageType");
			cmd = (RIGHTS_COMMANDS) msg.get("Command");
			...
			tempController = rc.getServerController().getController(receiver);
